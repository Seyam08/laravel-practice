    ...
    <div class="mx-auto max-w-2xl">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
            <div class="flex items-center gap-4 border-b border-gray-100 pb-6">
                <div
                    class="flex h-16 w-16 items-center justify-center rounded-full bg-gray-800 text-xl font-semibold text-white">
            ...
            <div class="mt-8 flex items-center gap-3 border-t border-gray-100 pt-6">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="rounded-lg border border-red-300 px-3 py-1.5 text-red-600 transition-colors hover:border-red-300 hover:bg-red-50 hover:text-red-600">
                        Logout
                    </button>
                </form>
                <a href="{{ route('books.index') }}"
                    class="rounded-lg px-4 py-2.5 text-sm font-medium text-gray-500 hover:text-gray-900">Back to
                    Books</a>
            </div>
        </div>
    </div>

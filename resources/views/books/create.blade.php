<x-layout title="Add Book">
    <div class="mx-auto max-w-5xl">
        ...
        <h1 class="mb-8 text-2xl font-bold text-gray-900">Create a New Book</h1>
        ...
        <form action="{{ route('books.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required
                    class="block w-full rounded-lg border border-gray-300 bg-slate-50 px-4 py-3 text-gray-900 focus:border-gray-400 focus:bg-white focus:outline-none focus:ring-0">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
